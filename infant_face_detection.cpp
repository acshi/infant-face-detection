...
int main(int argc, char** argv) {
	...
	add_image_left_right_flips(allImages, allFullFaces);
	// And then fix the part labels by making 0-5 become 5-0 for the mirrored ones
	for (int i = allFullFaces.size() / 2; i < allFullFaces.size(); i++) {
		std::vector<full_object_detection> faces = allFullFaces[i];
        for (int j = 0; j < faces.size(); j++) {
			full_object_detection face = faces[j];
			for (int k = 0; k < 3; k++) {
				point p = face.part(k);
				face.part(k) = face.part(5 - k);			
				face.part(5 - k) = p;
			}
		}
	}
	*/
	...
	// examine these images to see if anything fishy happened...
	//exhibit_face_detections(allImages, allFullFaces);
	//return 0;
	...
	/*exhibit_face_detections(trainingBaseImages, trainingFullFaces);
	cout << "NEXT" << endl;
	exhibit_face_detections(testingBaseImages, testingFullFaces);
	return 0;*/
	...
	*/
    ...
    for (uint32_t i = 0; i < testingBaseImages.size(); i++) {
        ...
        for (uint32_t j = 0; j < faces.size(); j++) {
            full_object_detection face = eyePredictor(testingBaseImages[i], faces[j].get_rect());
            //for (int k = 0; k < 6; k++) {
            ...
            win.add_overlay(image_window::overlay_rect(faces[j].get_rect(), rgb_pixel(0, 255, 0)));
			cin.get();
        }
    }
